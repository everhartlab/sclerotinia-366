...
northern bean, susceptible), Bunsi (aka Ex Rico, navy bean, low
...
has previously been shown that aggressiveness (the severity of disease
...
al., 2017). However, a question remains about the relationship between
mycelial compatibility groups and clonal genotypes.
...
a scale of 1--9) was significantly different with respect to Region,
MCG, or multilocus haplotype (MLH). To minimize complications due to
small sample sizes, we chose the top 10 MCGs, representing 56.5\% of the
isolates collected, the 10 most abundant MLHs representing 26.7\% of the
isolates, and populations with more than five isolates. If ANOVA results
were significantly different at \(\alpha\) = 0.05, pairwise differences
were assessed using Tukey's HSD test (\(\alpha\) = 0.05) using the
\texttt{HSD.test()} function in the package \emph{agricolae} version
1.2.8 (Mendiburu \& Simon, 2015).
...
loci). These 11 loci represented 7 chromosomes in the \emph{S.
...
the popualtions (table \ref{tab:iatab}). When we look at Mexico, we can
...
others was not an artifact. We believe that the differences we observe
from Mexico may reflect differences in climate.
...
phenotypes for aggressiveness should be evaluated for future studieds.
...
Denita Hadziabdic and two anonymous reviewers for their valuable
...
Dakota. \emph{North Dakota State University Extension}.
...
Ramasubramaniam, H., Río Mendoza, L. E. del, \& Bradley, C. A. (2008).
