...
We used ANOVA to assess if aggressiveness could be explained by Region, MCG, or multilocus haplotype (MLH). 
To minimize complications due to small sample sizes, we chose the top 10 MCGs, representing 56.5% of the isolates collected, the top 10 MLHs representing 26.7% of the isolates, and populations with more than five isolates.
...
In simple terms, this was done using a network-based approach where both MLHs and MCGs were considered nodes and the number of isolates in which they were found together was the strength of the connection between an MLH and and MCG node.
The network-based approach allowed us to assess how "clumpy" the relationships between MLHs and MCGs were.
In more specific terms, we first created a contingency table with MLHs and MCGs and converted it to a directed and weighted edgelist where each edge was a connection from an MCG to an MLH weighted by the number of samples shared in the connection. 
This was then converted to a bipartite graph where top nodes represented MCGs and bottom nodes represented MLHs.
The cluster walktrap community detection algorithm as implemented in the `cluster_walktrap()` function in *igraph* version `r packageVersion("igraph")` was used to define clusters of MLHs and MCGs [@csardi2006igraph; @pons2006computing].
...
Each edge represented a single MLG, but because a single MLH could be present in more than one population, that MLH would be represented by (*n*\*(*n*-1))/2 edges where n represents the number of populations crossed.
...
When we assessed the number of times two different MLHs that are in the same MCG, considering these as potential heterothallic pairings that could result in sexual recombination, we found an average of `r signif(mean(in_reach$N), 3)` potential heterothallic parings per MLG. 
...
We found significant population differentiation by geographic region, mainly differentiated into three broad North American groups based on shared haplotypes and posterior groupings, a Coastal Region, Midwestern Region, and Mexico. 
...
Either several MCGs belong to one MLG, which could be explained by insufficient sampling of loci; several MLHs belong to one MCG, which could be explained by clonal expansion; or a mixture of both.
...
This indicates that multilocus haplotypes are widespread across the United States, giving more evidence for a clonal population structure.
...
DAPC is based on the principal components of Euclidean distance between haplotypes [@jombart2010discriminant]. 
...
![
Graph showing complex associations between Mycelial Compatibility Groups (MCG) (dotted nodes) and Multilocus Haplotypes (MLH) (full nodes) where the number in each node represents the MLG/MCG assignment. 
