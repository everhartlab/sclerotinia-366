...
We additionally assessed within-locus allelic diversity by measuring Nei's gene diversity (*h*) [@nei1978estimation] and allelic evenness ($E_5$) [@pielou1975ecological; @grunwald2003analysis].
...
We assessed haplotype diversity by calculating Stoddart and Taylor's index (*G*) [@stoddart1988genotypic], Shannon's index (*H*) [@shannon2001mathematical], Simpson's index ($\lambda$) [@simpson1949measurement], evenness ($E_5$), and the expected number of multilocus haplotypes via rarefaction (*eMLH*) with 10 samples [@heck1975explicit; @hurlbert1971nonconcept; @pielou1975ecological; @grunwald2003analysis]. 
If all haplotypes in are equally abundant, both *G* and $e^H$ (the exponentiation of *H*) are expected to be equal to the number of haplotypes, $\lambda$ and $E_5$ are expected to equal one, and *eMLH* is expected to be at its maximum value (in this case, 10) [@grunwald2003analysis].
...
Locus `r loc_table$Locus[loc_table$allele==2]`, which contained only 2 alleles, showed low values of both *h* (`r signif(loc_table$Hexp[loc_table$allele==2], 3)`) and $E_5$ (`r signif(loc_table$Evenness[loc_table$allele==2], 3)`), indicating that there was one dominant allele present. 
...
Table: (\#tab:locus-stats) Allelic diversity on full data set at loci used in this study. *h* = Nei's 1978 gene diversity, $E_5$ = Evenness. Average *h* = `r signif(Hmean, 3)`, average $E_5$ = `r signif(Emean, 3)`, average no. alleles = `r signif(Amean, 3)`
