...
mx_percent   <- t(fa_alleles[, names(near_private)])/colSums(fa_alleles)
...
These protocols included three established cultivars used for comparison in the trials: Beryl (great northern bean, susceptible), Bunsi (aka Ex Rico, navy bean, low susceptibility), and G122 (cranberry bean, partial resistance) [@tu1982tolerance; @steadman2005identification; @otto-hanson2011variation].
...
We used ANOVA to assess if aggressiveness (determined via straw test on a scale of 1--9) was significantly different with respect to Region, MCG, or multilocus haplotype (MLH). 
...
These 11 loci represented 7 chromosomes in the *S. sclerotiorum* genome with a minimum distance of 55Kbp between two loci on the same chromosome.
...
<!-- 
The fact that genotypic diversity in poppr uses H, G and lambda should be clarified here. It should reflect the results interpretation as well. According to Table 2, MI and NE subpopulations exhibit high genotypic diversity when compared to other subpopulations (3.6, 29 and 0.97 for H, G and lambda, respectively for MI and 3.2, 17.8 and 0.94 for NE). If this was based on evenness, that should be explained as well. It can be confusing stated like this. When you shift focus onto diversity (h), the story changes – so please clarify this section to reflect that
-->
The results showed that, in terms of genotypic diversity (*H*, *G*, and $\lambda$), WA was the most diverse population with both *G* (`r signif(setNames(gdt$G, gdt$Pop)["WA"], 3)`) and $e^H$ (`r signif(setNames(exp(gdt$H), gdt$Pop)["WA"], 3)`) being close to the observed number of MLGs (`r setNames(gdt$MLG, gdt$Pop)["WA"]`). 
...
When we look at Mexico, we can observed that it had relatively high values of $E_5$ and genotypic diversity, but low richness, as measured by *eMLG*.
Moreover, Mexico had the lowest value for *h*, which is a measure of allelic diversity.
<!-- The results show that most populations exhibited relatively high amounts of genotypic diversity, with the exception of MI and NE, which have which exhibited lower-than average $E_5$ values. -->
...
A strip plot showing the distribution of severity across these three variables simultaneously can be seen in figure \@ref(mcg-mlh-region-aggressiveness).
...
Discriminant Analysis of Principal Components (DAPC) was performed by grouping Region with the first `r dapc_region$n.pca` principal components, representing `r prettify(dapc_region$var)`% of the total variance. 
...
While we found no evidence of private alleles in the Mexico and CA isolates, we did find that the alleles driving the first axis in figure \@ref(DAPC)A (`r mx_allele_string`, respectively) were overrepresented in Mexico (where >75% of the alleles came from the region).
...
Moreover, the fact that we find three alleles at three independent loci segregating for Mexico suggests that the pattern separating these populations from the others was not an artifact. 
...
<!-- I was also interested in more interpretation/analysis as to why the Mexican population did not have MLH shared with any other regions, the absence of clarity on this is done at a loss. -->
...
Additional phenotypes for aggressiveness should be evaluated for future studieds.
...
![
Example of MCG test plates showing (A) a compatible reaction with mycelia from two strains overgrowing each other and (B) an incompatible reaction with a barrage line of dead tissue forming between the two strains. Photo Credit: Rebecca Higgens.
