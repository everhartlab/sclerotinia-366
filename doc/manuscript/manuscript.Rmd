...
Locus `r locus_table$Locus[locus_table$allele==2]`, which contained only 2 alleles, showed low values of both *h* (`r signif(locus_table$Hexp[locus_table$allele==2], 3)`) and evenness (`r signif(locus_table$Evenness[locus_table$allele==2], 3)`), indicating that there was one dominant allele present. 
Analysis of the haplotype accumulation curve showed no clear plateau (Supplemental Information), indicating that we would likely obtain more multilocus haplotypes if we were to genotype more loci. 
...
The results show that most populations exhibited relatively high amounts of genotypic diversity, with the exception of MI and NE, which exhibited lower-than average $E_5$ values.
Mexico had the lowest average value of gene diversity, *h* and the lowest *eMLH*, suggesting low overall genetic diversity. 
...
Assessment of the marginal effects showed that all varaibles significantly explained genetic variation (*P* = `r anova1res[["Year"]]`, *P* = `r anova1res[["Region"]]` for Region).
...
![
Boxplot of aggressiveness by population arranged in descending order of mean aggressiveness for all populations with N > 5. Box boundaries represent 25th and 75th percentiles and whiskers extend to at most 1.5 $\times$ the interquartile range. Horizontal line represents median value. Circles represent individual isolates where filled circles are isolates from white mold screening nurseries (wmn) and open circles are isolates from producer fields.
](../../results/figures/publication/aggressiveness.pdf){#aggressiveness-boxplot}

<!-- end figure -->

Agressiveness of the isolates ranged from `r paste(range(other(dat11)$meta$Severity), collapse = " to ")` with a mean of `r signif(mean(other(dat11)$meta$Severity), 3)` and median of `r signif(median(other(dat11)$meta$Severity), 3)`.
...
We had performed an analysis on a network where the connections represented shared MLHs across populations, weighted by $1 - P_{sex}$ (Fig. \@ref(community-graph)). 
Community analysis of the MLHs shared between populations revealed 4 communities with a modularity of 0.17: A coastal community (CA, OR, WA, and NY), a midwest community (CO, NE, ND, MI), and an international community (MN, France, Australia).
...
The analysis of molecular variance (AMOVA) for clone-corrected samples over the hierarchy of Region, Source, and Year showed significant variation between regions and years, but no significant variation between wmn and non-wmn (Table \@ref(tab:amova)).
...
The North Central US populations (NE, MI, CO, ND) did not appear to have any variation across time in contrast to WA, which showed a shift in population structure in the last year of sampling, 2008 (Fig. \@ref(DAPC-RY)).
...
![
Scatter plot of Discriminant Analysis of Principal Components on Regions and Years showing non-differentiated temporal variation NE and MI and temporal variation in WA and CA. 
