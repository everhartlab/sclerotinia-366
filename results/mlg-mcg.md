...
the_communities <- data_frame(vertex = lay2$name, community = lay2$community) %>%
  group_by(community) %>%
  mutate(comm = ifelse(n() > 10, paste("Community", community), "Other Communities (n < 10)")) %>%
  mutate(comm = forcats::fct_infreq(comm) %>% forcats::fct_shift(-1L))
...
mcg_mlg_graph <- ggraph(lay2) +
  ...
  geom_node_text(aes(label = gsub("MLG.", "", name), 
                     color = ifelse(grepl("ther", community), "A", ifelse(grepl("3", community), "B", "C")), 
                     size = size/10), 
                 show.legend = FALSE) +
  coord_fixed() +
...
top5g <- make_mcgmlg_graph(top5) %>% add_communities(cluster_walktrap(g))
tosize <- V(top5g)$size
